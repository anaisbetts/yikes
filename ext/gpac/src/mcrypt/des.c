...
static GF_Err _mcrypt_set_key(DES_KEY * dkey, char *user_key, int len)
{
...
static void _mcrypt_encrypt(DES_KEY * key, char *block)
{
 ...
 * reverse order
 */
static void _mcrypt_decrypt(DES_KEY * key, char *block)
{
	register u32 left, right;
	register char *knp;
	u32 work[2];		/* Working data storage */
...
static u32 f(DES_KEY * key, register u32 r, register char *subkey)
{
	register u32 *spp;
